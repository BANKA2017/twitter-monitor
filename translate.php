<?php
/*
 from https://github.com/Stichoza/google-translate-php
*/
/**
...
class GoogleTokenGenerator
{
    ...
    private function TL($a)
    {
        ...
        for ($d = [], $e = 0, $f = 0; $f < self::JS_length($a); $f++) {
            $g = self::JS_charCodeAt($a, $f);
            if (128 > $g) {
                $d[$e++] = $g;
            } else {
                if (2048 > $g) {
                    ...
                    if (55296 == ($g & 64512) && $f + 1 < self::JS_length($a) && 56320 == (self::JS_charCodeAt($a, $f + 1) & 64512)) {
                        $g = 65536 + (($g & 1023) << 10) + (self::JS_charCodeAt($a, ++$f) & 1023);
                        $d[$e++] = $g >> 18 | 240;
                        $d[$e++] = $g >> 12 & 63 | 128;
                    } else {
                        $d[$e++] = $g >> 12 | 224;
