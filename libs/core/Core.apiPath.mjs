const path2array = (pathName = '', source = {}) => {
    const tmpPath = {
        ...
        "tweet_content": () => source?.content?.itemContent?.tweet_results?.result ?? source?.content?.content?.tweetResult?.result ?? source?.content?.itemContent?.tweet ?? source?.item?.itemContent?.tweet_results?.result ?? source?.item?.itemContent?.tweet ?? source?.content?.items[0]?.item?.content?.tweetResult?.result ?? false,
