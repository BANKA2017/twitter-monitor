...
const ApiLocalUserInfo = async (req, res) => {
    ...
    if (baseInfo === null) {
        ...
        res.json(apiTemplate(200, 'OK', Object.assign({}, tmpData, {description_origin: originTextAndEntities.originText, description_entities: originTextAndEntities.entities}), 'v3'))
    }
}
        ...
        if (!queryForConversation) {
            ...
            if (!queryForStatus) {
                ...
                switch (display) {
                    ...
                    case 'album':
                        ...
                        break
                    case 'space':
                        queryObject.push({tweet_id: {[Op.in]: dbHandle.twitter_monitor.literal(`(SELECT tweet_id FROM v2_twitter_cards WHERE type = 'audiospace' AND uid = ${dbHandle.twitter_monitor.escape(uid)} ${(hidden && uid !== '0' ? 'AND hidden = 0' : '')} AND tweet_id ${(tweetId === 0 ? '>' : (refresh ? '>' : '<')) + dbHandle.twitter_monitor.escape(tweetId)})`)}})
                        break
                    default:
            ...
            try {
                ...
                conversationList = await V2TwitterTweets.findAll({
                    attributes: ['tweet_id', 'origin_tweet_id', 'conversation_id_str', 'uid', 'name', 'display_name', 'media', 'video', 'card', 'poll', 'quote_status', 'source',  'full_text', 'full_text_origin', 'retweet_from', 'retweet_from_name', 'dispute', 'time'],
                    where: {
                        conversation_id_str: {[Op.eq]: dbHandle.twitter_monitor.literal(`(SELECT conversation_id_str FROM v2_twitter_tweets WHERE tweet_id = '${dbHandle.twitter_monitor.twitter_monitor.escape(tweetId)}')`)}
                    },
                    order: [['time']],
                    raw: true,
                })
...
const ApiLocalSearch = async (req, res) => {
    ...
    //query text
    const keyWord = VerifyQueryString(req.query.q, '').split(' ').filter(word => word !== '')
    ...
    if (advancedSearchMode) {
        ...
        //users
        const user = VerifyQueryString(req.query.user, '').replaceAll('@', '').split(' ').filter(word => word !== '')
...
const ApiLocalTag = async (req, res) => {
    ...
    let tweets = null
    try {
        tweets = await dbHandle.twitter_monitor.query("SELECT `tweet_id`, `origin_tweet_id`, `conversation_id_str`, `uid`, `name`, `display_name`, `media`, `video`, `card`, `poll`, `quote_status`, `source`,  `full_text`, `full_text_origin`, `retweet_from`, `retweet_from_name`, `dispute`, `time` FROM `v2_twitter_tweets` WHERE `tweet_id` = ANY(SELECT `tweet_id` FROM (SELECT `tweet_id` FROM `v2_twitter_entities` WHERE `text` = :hash AND `tweet_id` " + ((refresh || tweetId === 0) ? ">" : "<") + " :tweet_id AND `type` = :tag_type AND `hidden` = '0' ORDER BY `tweet_id` DESC LIMIT :count) AS t) ORDER BY `tweet_id` DESC", {
            replacements: {
                hash,
                tweet_id: tweetId,
                tag_type: tagType,
...
const ApiLocalHashtagRank = async (req, res) => {
    ...
    try {
        hashtagRank = await V2TwitterEntities.findAll({
            attributes: [[dbHandle.twitter_monitor.fn('COUNT', dbHandle.twitter_monitor.col('text')), 'value'], ['text', 'name']],
            where: {
                type: 'hashtag',
                hidden: 0,
                timestamp: {
...
const ApiLocalTrends = async (req, res) => {
    ...
    if (endData.length > 0) {
        ...
        res.json(apiTemplate(200, SQL_CONFIG.type === 'mysql' ? 'OK' : `Not supported ${SQL_CONFIG.dbtype}`, {
...
const GetUid = async (query) => {
    ...
    //uid first
    const tmpUserList = data_origin.users.filter(user => (
        (user.name !== '' && !(user.uid === '' || user.uid === 'undefined')) && 
        (
            (name.toLowerCase() === (user.name || '').toLowerCase()) && 
            (!isNaN(user.uid) && typeof user.uid !== 'object') || 
            (String(user.uid || '-1') === uid)
        )
    ))
...
const getDataFromTweets = async (tweets = [], count = 0, top = '0', historyMode = false, isRssMode = false, noUserName = false) => {
    ...
    if (isRssMode) {
        rss.channel({
            title: {text: noUserName ? 'Twitter Monitor Timeline' : `${tweets[0].name} 的 Twitter`, cdata: true},
            ...
            lastBuildDate: {text: (new Date()).toString().replaceAll(/\(.*\)/gm, '').trim(), cdata: false},
    ...
    for (let x = 0; x < realCount; x++) {
        ...
        if (tmpEntities && historyMode) {
            tweets[x].entities = tmpEntities.filter(entity => entity.tweet_id === tweets[x].tweet_id).map(entity => {
        ...
        if (tmpPollObject && tweets[x].poll) {
            tweets[x].pollObject = tmpPollObject.filter(poll => poll.tweet_id === tweets[x].tweet_id).map(poll => {
        ...
        if (tmpCardObject && tweets[x].card) {
            ...
            if (tmpCard.length > 0) {
                ...
                if (tmpCardApps && tweets[x].cardObject.unified_card_app) {
                    ...
                    tweets[x].cardObject.app = tmpCardApps.filter(app => app.tweet_id === tweets[x].tweet_id).map(app => {
                        delete app.tweet_id
                        return app
                    })
                } else {
        ...
        if (tmpMediaObject && (tweets[x].media || tweets[x].cardObject?.media || tweets[x].quoteObject?.media)) {
            //remove duplicate
            tweets[x].mediaObject = [...new Set(tmpMediaObject.filter(media => media.tweet_id === tweets[x].tweet_id).map(media => {
                ...
                if (media.source === 'tweets' && media.tweet_id === tweets[x].tweet_id) {
                    tmpImageText += `<img src="https://pbs.twimg.com/media/${media.filename}?format=${media.extension}&name=orig">`
                }
                return media
            }))]
        }
        ...
        if (isRssMode) {
            rss.item({
                ...
                description: {text: tweets[x].full_text.replaceAll('<br />', '<br>').replaceAll(/<a href="([^"]+)" id="([^"]+)"(| target="_blank")>([^<]+)<\/a>/gm, (...match) => match[1] === 'url' ? 0 : 3) + ' ' + tmpImageText, cdata: true},
                pubDate: {text: (new Date()).toString().replaceAll(/\(.*\)/gm, '').trim(), cdata: false},
