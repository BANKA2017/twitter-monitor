...
const ApiTypeahead = async (req, env) => {
    ...
    try {
        const tmpTypeaheadResponse = await getTypeahead({text, guest_token: req.guest_token2.token})
        //no rate limit
        ...
        return json(apiTemplate(500, 'Something wrong', {users: [],topics: []}, 'online'))
    }
...
const ApiListInfo = async (req, env) => {
    ...
    if (!(listId || (screenName && listSlug))) {
        return json(apiTemplate(403, 'Invalid Request', {}, 'online'))
    }

    try {
        let listInfoResponse = await getListInfo({id: listId ? listId : '', screenName, listSlug, guest_token: req.guest_token2.token, authorization: 1})
        //updateGuestToken
        await updateGuestToken(env, 'guest_token2', 1, listInfoResponse.headers.get('x-rate-limit-remaining') < 20)

        if (!listInfoResponse.data) {
            return json(apiTemplate(500, 'Songthing wrong', {}, 'online'))
        }
        if (listId) {
        ...
        return json(apiTemplate(200, 'OK', responseData, 'online'))
    } catch (e) {
        console.log(e)
...
const ApiListMemberList = async (req, env) => {
    ...
    try {
        let listMemberResponse = await getListMember({id: listId, cursor, count, guest_token: req.guest_token2.token, authorization: 1})
        //updateGuestToken
        await updateGuestToken(env, 'guest_token2', 1, listMemberResponse.headers.get('x-rate-limit-remaining') < 20)
