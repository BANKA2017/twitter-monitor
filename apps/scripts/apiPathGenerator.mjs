...
const apiPathList = {
    ...
    retweet_rest_id: [
        ...
        "legacy.retweeted_status.rest_id",
...
const codeText = Object.keys(apiPathList).map(typeName => `"${typeName}": () => ` + apiPathList[typeName].map(source => `source` + (source ? '?.' : '') + source.replaceAll('.', '?.')).join(' ?? ') + " ?? false").join(",\n        ")
