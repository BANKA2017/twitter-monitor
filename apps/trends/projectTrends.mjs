...
for (const config of trendsConfig) {
    ...
    const globalList = JSON.parse((await V2Config.findOne({
    ...
    //-->重跑时需要注意下一行的减号<--
    const guestLatestTwitterDataId = (await TwitterData.findOne({
        attributes: ["id"],
        order: [["id", 'DESC']],
        raw: true
    })).id - globalListCount * 11520
    ...
    for (const member of memberList) {
        ...
        userData.tweets.tag = Object.fromEntries(Object.keys(tmpTag).map(x => [x, tmpTag[x]]).sort((a, b) => b[1] - a[1]))
        userData.tweets.link = Object.fromEntries(Object.keys(tmpLink).map(x => [x, tmpLink[x]]).sort((a, b) => b[1] - a[1]))
